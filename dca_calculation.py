...
def dca_calculation(    dca_scheme, ph_symmetry=False,
                        Us = [1.0],
                        Ts = [0.125], 
                        ns = [0.5], fixed_n = True,
                        mutildes = [0.0],
  ...
  for p in ps:    
    ...
    actions =[  generic_action(  name = "lattice",
                ...
                generic_action(  name = "selfenergy",
                    ...
                    mixers = [], cautionaries = [], allowed_errors = [],    
                    printout = lambda data, it: (data.dump_general( quantities = ['SigmaR_iw','SigmaK_iw'], suffix='-current' ) if ((it+1) % print_current==0) else None)  )  ]
    ...
    monitors = [ monitor( monitored_quantity = lambda: dt.ns['00'], 
                          h5key = 'n_vs_it', 
    ...
    if (counter==0): #do the initial guess only once!         
      if initial_guess_archive_name!='':
        ...
        for K, sig in dt.SigmaK_iw:
          sig[0,0] << U/2.0
      dt.dump_general( quantities = ['SigmaK_iw','Sigma_imp_iw'], suffix='-initial' )  
